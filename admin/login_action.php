...
if(isset($_POST["admin_email_address"]))
{
	...
	if($total_row == 0)
	{
		$object->query = "
			...
			WHERE supplier_email_address = :admin_email_address
		";
		$object->execute($data);
		...
		else
		{
			...
			foreach($result as $row)
			{
				...
				{
					if($_POST["admin_password"] == $row["supplier_password"])
					{
						$_SESSION['admin_id'] = $row['supplier_id'];
						$_SESSION['type'] = 'supplier';
						$url = $object->base_url . '/admin/supplier_schedule.php';
					}
					else
					{
